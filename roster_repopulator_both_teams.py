#!/usr/bin/env python3
"""
Repopulate game_rosters table with complete roster data for BOTH teams.

This script:
1. Clears the existing game_rosters table
2. Fetches rosters for both home and away teams for each game
3. Populates with complete team information using the updated player_scrapper

The updated scraper now:
- Fetches rosters for both teams using team_id parameter
- Populates the team column with actual team names
- Ensures complete roster coverage for all games
"""
...
class RosterRepopulator:
    """Repopulates game roster database with complete roster data for both teams."""
    ...
    def clear_game_rosters(self) -> bool:
        """Clear existing game_rosters table."""
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()

            cursor.execute("DROP TABLE IF EXISTS game_rosters")
            conn.commit()
            conn.close()
            print(f"{GREEN}✓ Cleared existing game_rosters table{RESET}\n")
            return True
        except Exception as e:
            print(f"{RED}✗ Error clearing table: {e}{RESET}\n")
            return False
    ...
    def repopulate_rosters(self, game_ids: list) -> None:
        """Repopulate all game rosters with fresh API data for both teams."""
        print(f"\n{BLUE}{'='*70}{RESET}")
        print(f"{BOLD}Starting Roster Repopulation (Both Teams){RESET}")
        print(f"{BLUE}{'='*70}{RESET}\n")
        ...
        print(f"Processing rosters for BOTH teams per game...\n")
        ...
        for idx, game_id in enumerate(game_ids, start=1):
            ...
            try:
                # Scrape roster data for both teams
    ...
    def print_summary(self) -> None:
        ...
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM game_rosters;")
            total_entries = cursor.fetchone()[0]
            cursor.execute("SELECT COUNT(DISTINCT game_id) FROM game_rosters;")
            games_with_rosters = cursor.fetchone()[0]
            cursor.execute("SELECT COUNT(DISTINCT game_id, team) FROM game_rosters;")
            team_count = cursor.fetchone()[0]
            conn.close()

            print(f"\nRoster Statistics:")
            print(f"  Total roster entries: {total_entries}")
            print(f"  Games with rosters: {games_with_rosters}")
            print(f"  Average players per game: {total_entries / max(games_with_rosters, 1):.1f}")
        except:
            pass
    ...
    def run(self, limit: int = None) -> None:
        ...
        print(f"\n{CYAN}{BOLD}{'='*70}")
        print("AHL GAME ROSTER REPOPULATOR (BOTH TEAMS)")
        print(f"{'='*70}{RESET}\n")
        ...
        if not self.clear_game_rosters():
            print(f"{RED}Failed to clear rosters. Aborting.{RESET}")
            return
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Repopulate game_rosters table with complete roster data for both teams'
    )
    parser.add_argument(
        '--limit',
        type=int,
