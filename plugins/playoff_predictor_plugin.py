                ...
                try {
                    ...
                    for (const round of Object.keys(byRound).sort()) {
                        ...
                        for (const series of byRound[round]) {
                            ...
                            html += '<strong>' + series.series_label + '</strong><br>';

                            if (series.higher_seed_team_id && series.prediction) {
                                const pred = series.prediction;
                                const h_pct = Math.round(pred.home_series_win_pct);
                                const l_pct = Math.round(pred.away_series_win_pct);
                                const probClass = h_pct >= 70 ? 'prob-high' : (h_pct >= 40 ? 'prob-med' : 'prob-low');
                            ...
                            if (series.lower_seed_team_id && series.prediction) {
                                const pred = series.prediction;
                                const l_pct = Math.round(pred.away_series_win_pct);
                                const probClass = l_pct >= 70 ? 'prob-high' : (l_pct >= 40 ? 'prob-med' : 'prob-low');
...
async def playoffs_data_api(request, datasette):
    ...
    Returns the most recent playoff bracket, prioritizing actual playoffs over projections.
    Projections use playoff_season_id=999.
    """
    try:
        conn = get_db_connection()
        cursor = conn.cursor()

        # Get the latest playoff season - prefer actual playoffs, fall back to projections
        cursor.execute(
            """
            ...
            series_list.append(
                {
                    ...
                    "higher_seed_team_id": row["higher_seed_team_id"],
                    "lower_seed_team_id": row["lower_seed_team_id"],
                    "prediction": prediction,
                }
            )

            # Track team names
            if row["higher_seed_team_id"]:
                team_names[row["higher_seed_team_id"]] = row["higher_team_name"]
            if row["lower_seed_team_id"]:
                team_names[row["lower_seed_team_id"]] = row["lower_team_name"]
